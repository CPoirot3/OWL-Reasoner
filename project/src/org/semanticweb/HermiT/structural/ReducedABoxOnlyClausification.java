...
public class ReducedABoxOnlyClausification extends OWLAxiomVisitorAdapter {
    ...
    public ReducedABoxOnlyClausification(Configuration configuration, OWLDataFactory factory,
                                         Set<AtomicConcept> allAtomicConcepts, Set<AtomicRole> allAtomicObjectRoles,
    ...
    protected Atom getRoleAtom(OWLObjectPropertyExpression objectProperty, Term first, Term second) {
        ...
        if (m_allAtomicObjectRoles.contains(atomicRole))
            ...
            throw new IllegalArgumentException(
                    "Internal error: fresh properties in property assertions are not compatible with incremental ABox loading!");
    }
    ...
    public void visit(OWLSameIndividualAxiom object) {
        ...
        for (int i = 0; i < individuals.length - 1; i++)
            m_positiveFacts.add(
                    Atom.create(Equality.create(), getIndividual(individuals[i]), getIndividual(individuals[i + 1])));
    }
    ...
    public void visit(OWLDifferentIndividualsAxiom object) {
        ...
        for (int i = 0; i < individuals.length; i++)
            for (int j = i + 1; j < individuals.length; j++)
                m_positiveFacts.add(
                        Atom.create(Inequality.create(), getIndividual(individuals[i]), getIndividual(individuals[j])));
    }

    public void visit(OWLClassAssertionAxiom object) {
        // we can handle everything that results in positive or negative facts
        // (C a) with C a named class, HasSelf, HasValue, negated named class,
        // negated HasSelf, negatedHasValue
        ...
        if (description instanceof OWLClass) {
            ...
            m_positiveFacts.add(getRoleAtom(((OWLObjectHasSelf) description).getProperty().getNamedProperty(),
                    getIndividual(object.getIndividual()), getIndividual(object.getIndividual())));
            ...
            if (negated instanceof OWLClass) {
                ...
                m_negativeFacts.add(getRoleAtom(((OWLObjectHasSelf) negated).getProperty().getNamedProperty(),
                        getIndividual(object.getIndividual()), getIndividual(object.getIndividual())));
                ...
                throw new IllegalArgumentException(
                        "Internal error: invalid normal form for ABox updates (class assertion with negated class).");
            }
        } else
    ...
    public void visit(OWLObjectPropertyAssertionAxiom object) {
        m_positiveFacts.add(getRoleAtom(object.getProperty(), getIndividual(object.getSubject()),
                getIndividual(object.getObject())));
    }

    public void visit(OWLNegativeObjectPropertyAssertionAxiom object) {
        m_negativeFacts.add(getRoleAtom(object.getProperty(), getIndividual(object.getSubject()),
                getIndividual(object.getObject())));
    }
    ...
    protected Constant getConstant(OWLLiteral literal) {
        try {
            if (literal.isRDFPlainLiteral()) {
                if (literal.hasLang())
                    return Constant.create(literal.getLiteral() + "@" + literal.getLang(),
                            Prefixes.s_semanticWebPrefixes.get("rdf:") + "PlainLiteral");
                else
                    return Constant.create(literal.getLiteral() + "@",
                            Prefixes.s_semanticWebPrefixes.get("rdf:") + "PlainLiteral");
            } else
