...
/**
 * This class is responsible for abbreviating IRIs. The resulting IRIs can be
 * either<br>
 * 1) &lt;uri&gt; or<br>
 * 2) prefix-name:local-name where prefix-name can be empty.<br>
 * Forms 1 and 2 are dependent upon a set of prefix declarations that associates
 * prefix names with prefix IRIs. A IRI abbreviated using form 2 that uses an
 * unregistered prefix is invalid---expanding it will result in an exception.
 * Neither prefixes nor local names may contain colon characters. The grammar
 * used for various parts of the IRIs is as follows:<br>
 * PN_CHARS_BASE ::= [A-Z] | [a-z] | [#x00C0-#x00D6] | [#x00D8-#x00F6] |
 * [#x00F8-#x02FF] | [#x0370-#x037D] | [#x037F-#x1FFF] | [#x200C-#x200D] |
 * [#x2070-#x218F] | [#x2C00-#x2FEF] | [#x3001-#xD7FF] | [#xF900-#xFDCF] |
 * [#xFDF0-#xFFFD] | [#x10000-#xEFFFF]<br>
 * PN_CHARS ::= PN_CHARS_BASE | '_' | '-' | [0-9] | #x00B7 | [#x0300-#x036F] |
 * [#x203F-#x2040]<br>
...
public class Prefixes implements Serializable {
    ...
    protected static final Pattern s_localNameChecker = Pattern
            .compile("(" + PN_CHARS_BASE + "|_|[0-9])((" + PN_CHARS + "|[.])*(" + PN_CHARS + "))?");
    ...
    /**
     * Expands a full IRI from the abbreviated one, which is of one of the
     * following forms: 'prefix:name', where 'prefix' is a registered prefix
     * name (can be empty), or '&lt;iri&gt;', where 'iri' is an IRI.
     */
    public String expandAbbreviatedIRI(String abbreviation) {
        if (abbreviation.length() > 0 && abbreviation.charAt(0) == '<') {
            if (abbreviation.charAt(abbreviation.length() - 1) != '>')
                throw new IllegalArgumentException("The string '" + abbreviation
                        + "' is not a valid abbreviation: IRIs must be enclosed in '<' and '>'.");
            ...
            if (pos != -1) {
                ...
                if (prefixIRI == null) {
                    ...
                    // http:
                    if (prefix == "http:")
                        throw new IllegalArgumentException("The IRI '" + abbreviation
                                + "' must be enclosed in '<' and '>' to be used as an abbreviation.");
...
//        System.out.println("bbb " + prefixName);
     ...
     *
     * @param individualIRIs the collection of IRIs used in individuals (used for
     *                       registering nominal prefix names)
     * @return 'true' if this object already contained one of the internal
     * prefix names
     */
    public boolean declareInternalPrefixes(Collection<String> individualIRIs, Collection<String> anonIndividualIRIs) {
        ...
        for (String iri : individualIRIs) {
            if (declarePrefixRaw("nom" + (individualIRIsIndex == 1 ? "" : String.valueOf(individualIRIsIndex)) + ":",
                    "internal:nom#" + iri))
                containsPrefix = true;
            individualIRIsIndex++;
        }
        ...
        for (String iri : anonIndividualIRIs) {
            if (declarePrefixRaw("anon"
                            + (anonymousIndividualIRIsIndex == 1 ? "" : String.valueOf(anonymousIndividualIRIsIndex)) + ":",
                    "internal:anon#" + iri))
                containsPrefix = true;
     ...
     *
     * @return 'true' if this object already contained one of the well-known
     * prefixes
     */
     ...
     * @return 'true' if this object already contained one of the prefixes from
     * the supplied object
     */
